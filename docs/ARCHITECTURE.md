...
- Root `action.yml` defines a GitHub Action boundary that compares PR base/head fixture reports, posts score deltas for `reusability` and `accessibility_risk` with explicit regression-gate status, validates strict boolean regression-gate input, exports deterministic gate/delta/reason outputs for workflow reuse, and can optionally fail on score regression after commenting
